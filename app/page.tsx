...
import { futureMenuWeeks, futureExtras, futureWeekTotals, futurePlanTotal } from "./future-plan-data";
...
export default function Home(){
 ...
 const history=weekNumber<=3?getMealHistoryByMenuKey(`${weekNumber}:${meal.title}`):undefined;
 return <button className="meal meal-with-history" type="button" aria-haspopup="dialog" key={index} onClick={event=>{setRecipeTrigger(event.currentTarget);setSelected(meal);}}><span className="day">{meal.day}<small>{meal.type}</small></span><span className="dish">{meal.title}<small>{meal.batch}</small>{history&&<small className="meal-history-note">{history.note}</small>}</span><span className="meal-action"><MealHistoryBadges outcome={history?.outcome??(isHistorical?"unconfirmed":"planned")} preference={history?.preference??null}/><span className="open">Рецепт</span></span></button>;
 })}</div>
 {!isHistorical&&<details className="expenses week-extras"><summary>Перекусы и подготовка недели</summary><div className="details-content"><ul>{current.prep.map(s=><li key={s}>{s}</li>)}</ul>{futureExtras[weekNumber-5].map(extra=><article key={extra.title}><h3>{extra.title}</h3><p>{extra.how}</p><small>{extra.quantity}</small></article>)}</div></details>}
