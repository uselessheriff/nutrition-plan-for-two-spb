...
if (Math.abs(archivedActualTotal - confirmedTotal) > 0.01) {
  throw new Error(`Фактические итоги недель (${archivedActualTotal}) не совпадают с покупками (${confirmedTotal}).`);
}
    ...
    <>
      ...
      <main id="top">
        <section className="hero shell">
          <div className="hero-copy"><span className="eyebrow">Обновлено 29 августа · 3 недели факта</span><h1>Питание на месяц для двоих</h1><p>Недели 1–3 сохранены вместе с подробными рецептами. Неделя 4 пересобрана по фактическим остаткам: замороженные рыба и фарши идут первыми, гарниры чередуются, а рис используется гарниром один раз.</p></div>
          <aside className="budget-card"><div className="budget-head"><span>Текущий бюджет</span><strong>{money(budget)}</strong></div><div className="meter"><span style={{ width: `${Math.min(100, plannedMonth / budget * 100)}%` }} /></div><div className="budget-numbers"><div><span>Факт за 3 недели</span><strong>{exact(confirmedTotal)}</strong></div><div><span>Коридор месяца</span><strong>{money(forecastLow)}–{money(forecastHigh)}</strong></div></div><p>Осталась одна чистая корзина недели 4: крупы, яйца, картофель и оба сыра повторно не покупаются.</p></aside>
        </section>

        <section className="section shell" id="fact">
          <div className="section-heading"><div><span className="eyebrow">Три прошедшие недели</span><h2>Подтверждённые траты</h2></div><p>В расчёт внесены товарные строки чеков и сообщённые докупки. Чек 21.08 отнесён к третьей неделе по последнему уточнению и не посчитан повторно.</p></div>
          <div className="stats">
            <article className="stat stat-primary"><span>Рацион · факт</span><strong>{exact(confirmedTotal)}</strong><small>Н1 — {exact(weekOneTotal)}, Н2 — {exact(weekTwoTotal)}, Н3 — {exact(weekThreeTotal)}.</small></article>
            <article className="stat"><span>Неделя 3 · факт</span><strong>{exact(weekThreeTotal)}</strong><small>Чеки 21–22.08 и {exact(1458)} дополнительных покупок.</small></article>
            ...
            <article className="stat"><span>Корзина недели 4</span><strong>{money(futureTotal)}</strong><small>Оценка только недостающих продуктов после вычета остатков.</small></article>
          </div>
          <div className="fact-grid">
            ...
            <div className="notes">
              <article className="note success"><span>Проверено</span><h3>Третья неделя закрыта</h3><p>21 августа — {exact(783.57)}, чек «Ленты» 22 августа — {exact(4319.58)}, дополнительные покупки — {exact(1458)}. Вместе — {exact(weekThreeTotal)}.</p></article>
              <article className="note"><span>Без задвоения</span><h3>Восемь позиций уже были в базе</h3><p>Тортильи, лепёшка, лимонный сок, перец, бананы, томаты, картофель и молоко из списка совпали с точным чеком 21.08 на {exact(783.57)}; они только перенесены в неделю 3.</p></article>
        ...
        <section className="section tint analysis-section" id="analysis"><div className="shell">
          ...
          <div className="analysis-stats"><article><span>Средняя неделя</span><strong>{exact(observedWeeklyAverage)}</strong><small>Факт трёх недель.</small></article><article><span>Линейный темп месяца</span><strong>{exact(runRateMonth)}</strong><small>Если средняя трёх недель сохранится.</small></article><article><span>Стартовый остаток</span><strong>{stock.length} позиций</strong><small>Только продукты, прямо перечисленные пользователем.</small></article></div>
          ...
          <p className="method-note"><strong>Метод.</strong> *Цена за 10 г белка и за 100 ккал — ориентир на типичные пищевые значения продуктов, а не данные чеков. Покупка овощей и фруктов в лавке на {exact(1035)} включена в общий факт и категорию, но не в нормализованные цены: разбивки по товарам и весу нет.</p>
        </div></section>

        <section className="section shell" id="stock"><div className="section-heading"><div><span className="eyebrow">Старт недели 4</span><h2>Что уже есть дома</h2></div><p>Сначала используются размороженные в холодильнике рыба и оба фарша, затем овощное рагу, свежие овощи, яйца и сыры. Сухие крупы остаются резервом.</p></div><div className="stock-grid">{stock.map((item) => <div className="stock-item" key={item.name}><span>{item.name}</span><strong>{item.quantity}</strong><small className={`stock-priority ${item.priority}`}>{item.priority === "сначала" ? "используем раньше" : "долгий запас"}</small></div>)}</div><p className="footnote">Остатки зафиксированы со слов пользователя 29 августа. Рыба и оба фарша находятся в морозилке; размораживайте их в холодильнике. Неупомянутые остатки не предполагались.</p></section>

        <section className="section shell" id="weeks">
          <div className="section-heading"><div><span className="eyebrow">Меню и архив</span><h2>Недели 1–4</h2></div><p>Недели 1–3 — архив факта и подробных рецептов, неделя 4 — текущий план. Порции рассчитаны на двух взрослых: ориентир основного ужина — около 550–750 ккал на человека, но точность зависит от бренда, масла и фактической порции.</p></div>
        ...
        <section className="section forecast"><div className="shell forecast-card"><div><span className="eyebrow">Прогноз месяца</span><h2>{money(forecastLow)}–{money(forecastHigh)}</h2><p>Базовая сумма месяца — {exact(plannedMonth)}: три недели факта и корзина недели 4. Коридор ±1 000 ₽ учитывает колебание цен, питьевое молоко, фрукты и мелкие докупки; он не является гарантией.</p></div><div className="forecast-weeks">{archivedWeeks.map((week) => <div key={week.number}><span>Неделя {week.number} · факт</span><strong>{exact(week.actualTotal ?? 0)}</strong></div>)}{futureWeeks.map((week) => <div key={week.number}><span>Неделя {week.number} · план</span><strong>{money(total(week.shopping))}</strong></div>)}</div><p className="method"><strong>Факт:</strong> {exact(confirmedTotal)} за три недели. <strong>Вывод:</strong> четвёртая корзина снижена за счёт замороженных рыбы и фаршей, 11 яиц, двух сыров, картофеля и круп. <strong>Гипотеза:</strong> при близких ценах месяц уложится в бюджет с резервом не менее {money(Math.max(0, budget - forecastHigh))}.</p></div></section>
      </main>
