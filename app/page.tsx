...
type Week = { number: number; title: string; focus: string; prep: string[]; meals: Meal[]; shopping: ShopItem[] };
...
const baseWeeks: Week[] = [
  ...
  {
    number: 3,
    title: "Курица без однообразия",
    focus: "Одна большая закупка белка превращается в шесть разных блюд; овсянка и лён продолжают расходоваться из запаса.",
    prep: ["Запечь целую курицу и снять мясо с костей.", "Половину фарша оставить для перцев, из половины сформировать котлеты.", "Сварить рис на первые три дня.", "Пирог сделать на 3 порции: одна остаётся на воскресенье."],
    meals: [
      meal("Понедельник", "Ужин", "Запечённая курица с рисом и овощами", "3 порции", "50 мин", "Курица — 700 г|Рис — 220 г сухого|Овощи — 700 г|Лук — 1 шт.", "Курицу запекайте до полной готовности.|Сварите рис, овощи запеките рядом.|Остаток мяса снимите с костей для следующих блюд."),
      meal("Вторник", "Ужин", "Перцы с индейкой и рисом", "3 порции · одна в ланч-бокс", "55 мин", "Фарш — 350 г|Перец — 600 г|Рис — 120 г|Пассата — 300 г|Сыр — 80 г", "Смешайте фарш с полуготовым рисом.|Наполните перцы и добавьте пассату.|Запекайте 35–40 минут, в конце добавьте сыр."),
      ...
      meal("Пятница", "Ужин", "Кесадилья с курицей и сыром", "3 порции", "25 мин", "Готовая курица — 350 г|Тортильи — 6 шт.|Сыр — 150 г|Помидоры — 300 г|Перец — 200 г", "Нарежьте начинку.|Разложите её с сыром между тортильями.|Подрумяньте с двух сторон."),
    ...
    shopping: [
      ...
      shop("Молочное", "Яйца С1", "10 шт. сверх остатка", 150), shop("Молочное", "Творог и сыр", "500 г + 400 г", 500),
      ...
      shop("Фрукты", "Фрукты", "2,7 кг", 520), shop("Бакалея", "Рис и паста", "500 г + 500 г", 280),
      shop("Бакалея", "Тортильи и хлеб", "8 шт. + 1 буханка", 280), shop("Бакалея", "Тесто для пирога", "500 г", 120),
      shop("Бакалея", "Пассата, соусы и специи", "1 набор", 250), shop("Овощи", "Салатные листья, зелень и лимон", "1 набор", 80),
      shop("Удовольствия", "Сок и 4 молочных десерта", "1,5 л + 4 шт.", 450), shop("Доставка", "Слот и пакеты", "1 заказ", 100),
    ],
  },
  {
    number: 4,
    title: "Рыба, запеканки и быстрые остатки",
    focus: "Один рыбный вечер, две заготовки на следующий приём пищи и минимум продуктов, которые нельзя заморозить.",
    prep: ["Рыбу готовить только в понедельник, печень — во вторник.", "Кебабы замариновать утром среды.", "Запеканки сразу разделить на порции.", "Пиццу делить на 4 части и хранить остаток только до следующего приёма пищи."],
    meals: [
      ...
      meal("Пятница", "Ужин", "Домашняя пицца с курицей", "4 части", "45 мин", "Тесто — 500 г|Курица — 400 г|Пассата — 180 г|Сыр — 160 г|Помидоры и перец — 350 г", "Курицу заранее приготовьте.|Разложите соус, начинку и сыр по тесту.|Выпекайте 14–17 минут при 220 °C."),
      ...
      meal("Воскресенье", "Ужин", "Курица с картофелем и овощами", "3 порции · одна на понедельник", "50 мин", "Куриное филе — 500 г|Картофель — 800 г|Овощи — 650 г|Лук — 1 шт.|Лимон — 1/2 шт.", "Картофель и плотные овощи запекайте 15 минут.|Добавьте курицу и остальные овощи ещё на 25–30 минут.|Третью порцию уберите."),
    ],
    shopping: [
      shop("Белок", "Форель", "600 г", 650), shop("Белок", "Куриное филе", "1,8 кг", 800), shop("Белок", "Печень", "450 г", 280),
      shop("Белок", "Фарш индейки", "1 кг", 600), shop("Молочное", "Яйца С1", "10 шт.", 150), shop("Молочное", "Творог и сыр", "400 г + 450 г", 600),
      shop("Молочное", "Молоко и йогурт", "1,7 л + 500 г", 300), shop("Овощи", "Картофель, лук, морковь и капуста", "4,5 кг", 350),
      shop("Овощи", "Помидоры, огурцы, перец и зелень", "3 кг", 520), shop("Овощи", "Замороженные овощи", "1 кг", 220),
      shop("Фрукты", "Фрукты", "2,2 кг", 400), shop("Бакалея", "Рис, булгур и паста", "400 г + 400 г + 500 г", 330),
      shop("Бакалея", "Хлеб, тортильи и тесто", "1 буханка + 6 шт. + 500 г", 400), shop("Бакалея", "Пассата, специи, горчица и лимон", "1 набор", 220),
      ...
      <main id="top">
        ...
        <section className="section shell" id="fact">
          ...
          <div className="stats">
            ...
            <article className="stat"><span>Следующие 3 недели</span><strong>{money(futureTotal)}</strong><small>5 260 ₽ + 5 775 ₽ + 6 250 ₽.</small></article>
        ...
        <section className="section forecast"><div className="shell forecast-card"><div><span className="eyebrow">Прогноз месяца</span><h2>{money(forecastLow)}–{money(forecastHigh)}</h2><p>Лимит 25 000 ₽ достижим, но верхняя граница почти выбирает весь запас.</p></div><div className="forecast-weeks"><div><span>Неделя 1 · факт</span><strong>{exact(confirmedTotal)}</strong></div>{weeks.map((week) => <div key={week.number}><span>Неделя {week.number} · план</span><strong>{money(total(week.shopping))}</strong></div>)}</div><p className="method"><strong>Факт:</strong> 5 048,99 ₽ подтверждено. <strong>Вывод:</strong> итог зависит от спонтанных докупок и двух неизвестных цен. <strong>Гипотеза:</strong> при повторении молока и десертов, но без новой порчи, месяц завершится в этом диапазоне.</p></div></section>
      </main>
